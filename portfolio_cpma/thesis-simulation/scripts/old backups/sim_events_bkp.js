...
function init() {
   ...
   //hide school info, selection, and prediction boxes until user selects a school
	...
	var selectionBox = document.getElementById("model-select").style.display = 'none';
...
function predictScore(evt) {
	...
	evt.preventDefault();
	evt.stopPropagation();
	//display area where data will appear
	var predictionBox = document.getElementById("prediction").style.display = 'block';
   ...
   var actualScore = 	"<p><span class=\"highlight\">Actual Score 2012-2013: </span>" + schoolInfo[score2013] + "</p>";
	...
	document.getElementById("prediction").innerHTML = predictedScoreModel + predictInterval + confInterval + actualScore;
	
	//add event listeners for the links in the prediction box, which will display an explanation in a popup window on mouseover
...
function predictPass(evt) {
	...
	//declare constants
	var actualProbPass = 0.73; //'188/257'
	...
	if(schoolInfo[score2013] < 70) {
		var actualResult = "Fail";	
	}
	else actualResult = "Pass";
   ...
   var logOdds = Math.round(logOdds*1000)/1000;
	...
	if(probPass < 0.5) {
		var predictedResult = "Fail";	
	}
	else predictedResult = "Pass";
	
	//display results in the prediction box
	document.getElementById("prediction").innerHTML = 
	"<a id=\"prob-predict-link\" href=\"\"><p><span class=\"highlight\">Probability of Passing from Model: </span>" + probPass + "%</p></a>" +
	"<a id=\"logistic-predict-link\" href=\"\"><p><span class=\"highlight\">Prediction from Model: </span>" + predictedResult + "</p></a>" +
	"<a id=\"prob-actual-link\" href=\"\"><p><span class=\"highlight\">Probability of Passing from Data: </span>" + actualProbPass*100 + "%</p></a>" +
   	"<p><span class=\"highlight\">Actual Score 2012-2013: </span>" + schoolInfo[score2013] + " (" + actualResult + ")</p>";
   	
   	//add event listener for the link in the prediction box, which will display an explanation in a popup window on mouseover
